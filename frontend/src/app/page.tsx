...
export default function Home() {
  ...
  const handleStartLocalTracking = async () => {
    if (!localDirectoryPath.trim()) {
      alert("Please enter the full path to the local directory you want to track.");
      return;
    }
    ...
    try {
      const response = await fetch("http://localhost:8000/start-local-tracking", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      alert("Failed to start local directory tracking. Please check the path and try again.");
    }
  };
  ...
  const handleStartTracking = async () => {
    ...
    if (trackingMode === "both" && (!localDirectoryPath.trim() || !gitRepoPath.trim())) {
              ...
              <CardContent>
                {events.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-xs mt-2">Debug: Events array length: {events.length}</p>
                  </div>
                ) : (
          ...
          <TabsContent value="repo" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Choose what you want to track: local directory, Git repository, or both
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <Button
                        variant={trackingMode === "local" ? "default" : "outline"}
                        onClick={() => setTrackingMode("local")}
                        className="flex flex-col items-center gap-2 h-auto py-4"
                      >
                        <FolderOpen className="h-5 w-5" />
                        <span className="text-sm">Local Directory</span>
                        <span className="text-xs opacity-70">File changes only</span>
                      </Button>
                      <Button
                      ...
                      >
                        ...
                        <span className="text-xs opacity-70">Commits & changes</span>
                      </Button>
                      <Button
                        variant={trackingMode === "both" ? "default" : "outline"}
                        onClick={() => setTrackingMode("both")}
                        className="flex flex-col items-center gap-2 h-auto py-4"
                      >
                        <Activity className="h-5 w-5" />
                        <span className="text-sm">Both</span>
                        <span className="text-xs opacity-70">Complete tracking</span>
                      </Button>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <Input
                          ...
                          className="flex-1"
                          onKeyPress={(e) => e.key === 'Enter' && handleStartLocalTracking()}
                        />
                        <Button 
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Enter the full path to any directory you want to track for file changes
                      </p>
                    </div>
                  )}
                  ...
                  <div className="border-t pt-4">
                    ...
                    />
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                      Note: File picker will populate the Local Directory Path field
                    </p>
                  </div>
